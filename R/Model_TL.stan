...
transformed parameters {
  // real mu_nat;
  // real<lower=-10, upper=2> mu_nat;  // corresponds roughly to FOI in (0, 0.88)
  ...
  // --- VILLAGE LEVEL ---
  for (v in 1:V) {
    mu_v[v] = national_mu + delta_v[v];           // logit of FOI
...
model {
  ...
  se ~ normal(0.77, 0.05); //sensitivity around different sd
  sp ~ normal(0.93, 0.05); //sensitivity around different sd
